...
const Survey: React.FC = () => {
  ...
  const renderLikertSection = (title: string, questions: { field: keyof UserDemographics, text: string }[]) => (
    <div className="bg-white rounded-lg border border-slate-200 shadow-sm mb-4 overflow-hidden">
      ...
      <div className="p-0">
        {questions.map((q, idx) => (
          <div key={q.field} className={`px-6 py-5 flex flex-col md:flex-row items-start md:items-center ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'}`}>
            <div className="w-full md:w-1/2 text-base text-slate-700 leading-snug pr-4 mb-3 md:mb-0">{q.text}</div>
  ...
  <h3 className="text-lg md:text-lg font-black text-white uppercase tracking-tight mb-4">KHẢO SÁT VỀ TÌNH CẢM VÀ KIẾN THỨC MÔI TRƯỜNG TRƯỚC THỰC NGHIỆM</h3>
